let player, yellowCircles = [], blueCircles = [], whiteCircles = [], purpleCircles = [], 
    ...
    startTime, playerScore = 0, highScore = 0, menuMusic, gameMusic = [], 
...
function gameOver() {
  ...
  fill(255);
  textSize(24);
  text("Final Score: " + playerScore, width/2, height/2 + 50);
...
function drawGameOverScreen() {
  ...
  rectMode(CENTER);
  fill(255, 0, 0);
  rect(againButton.x + againButton.width/2, againButton.y + againButton.height/2, 
  ...
  text(againButton.label, againButton.x + againButton.width/2, 
       againButton.y + againButton.height/2);
...
function mousePressed() {
  ...
  if (gameState === 'gameover' && 
      ...
      mouseY > againButton.y && mouseY < againButton.y + startButton.height) {
    resetGame();
    startGame();
  }
}